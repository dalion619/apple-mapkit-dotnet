...
namespace Apple.MapKit.RestClient.Interfaces
{
    ...
    public interface ISearch {
        [Get("/search?q={query}")]
        Task<GenericResponseModel> Search(BaseRequestModel model, string query);
        
        [Get("/searchAutocomplete?q={query}")]
        Task<SearchAutocompleteResponseModel> SearchAutocomplete(BaseRequestModel model, string query);
    }
}
